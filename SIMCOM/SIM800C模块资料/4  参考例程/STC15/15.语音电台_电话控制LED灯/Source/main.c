...
void Check_New_Message(void)
{
    ...
    }else
    {
        if(strstr(Uart2_Buf,"+DTMF:")!=NULL)   		//有按键按下
        {
            ...
            if(strstr(Uart2_Buf,"1")!=NULL)//打开LED
            {
                LED=1;
                Second_AT_Command("AT+CTTS=2,\"打开LED\"",3); //播放语音测试
                UART1_SendString("打开LED\r\n");
            }else
            if(strstr(Uart2_Buf,"2")!=NULL)//关闭第一路
            {
                LED=0;
                Second_AT_Command("AT+CTTS=2,\"关闭LED\"",3); //播放语音测试
                UART1_SendString("关闭LED\r\n");
            }
            CLR_Buf2();
        }else
