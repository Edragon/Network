...
void Set_TTS_Mode(void); //设置短信为TEXT模式
...
void Set_TTS_Mode(void)
{
	...
	Second_AT_Command("AT+CTTSPARAM=100,0,50,50,1","OK",3);					//设置TTS参数
	Second_AT_Command("AT+DDET=1","OK",3);								    //打开DTMF解码
}
...
void Check_New_Message(void)
{
    ...
    if(Ring_flag==0)
    {
        if(strstr(Uart1_Buf,"RING")!=NULL)   		//若缓存字符串中含有"+CMT"就表示有新的短信
        {
            Second_AT_Command("ATA","OK",3);             //接听电话
            Ring_flag=1;
            Second_AT_Command("AT+CTTS=2,\"欢迎使用泥人电子语音测试程序\"","OK",3); //播放语音测试
            CLR_Buf1();
        }
    }else
    {
        if(strstr(Uart1_Buf,"+DTMF:")!=NULL)   		//有按键按下
        {
            if(strstr(Uart1_Buf,"1")!=NULL)//打开LED
            {
                LED=1;
                Second_AT_Command("AT+CTTS=2,\"打开LED\"","OK",3); //播放语音测试
            }else
            if(strstr(Uart1_Buf,"2")!=NULL)//关闭第一路
            {
                LED=0;
                Second_AT_Command("AT+CTTS=2,\"关闭LED\"","OK",3); //播放语音测试
            }
            CLR_Buf1();
        }else
        if(strstr(Uart1_Buf,"NO CARRIER")!=NULL)
        {
            Ring_flag=0;
            CLR_Buf1();
        }
